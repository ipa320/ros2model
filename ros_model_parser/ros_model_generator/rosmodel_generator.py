...
class RosModelGenerator(object):
    ...
    def create_model_from_node(
        self, package_name, artifact_name, node, repo=None, pkg_type="CatkinPackage"
    ):
    ...
    def generate_ros_model_from_system(
        self, rossystem, package, ros_model_file, print_param_value=True
    ):
        sucess, ros_model_str = self.create_ros_model_from_system(
            package, rossystem, print_param_value
        )
        with open(ros_model_file, "w") as outfile:
            outfile.write(ros_model_str)
    ...
    def create_ros_model_from_system(
        self, package_name, rossystem, print_param_value=True
    ):
        ...
        for component in rossystem.components:
            ...
            for param in component.params:
                node.add_parameter(
                    param.resolved, None, None, param.value, print_param_value
                )
