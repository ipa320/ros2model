...
class RosSystemModelGenerator(object):
        ...
        for name in components:
            ...
            if "parameters" in components[name]:
                ...
                for param_name, param in parameters.items():
                    component.params.add(
                        system_model.RosParameter(param_name, param[1], param[0])
                    )
            ...
            if "service_servers" in components[name]:
                ...
                for serv, serv_type in service_servers.items():
                    component.service_servers.add(
                        system_model.RosInterface(serv, serv_type)
                    )
            ...
            if "action_clients" in components[name]:
                ...
                for action, action_type in action_clients.items():
                    component.action_clients.add(
                        system_model.RosInterface(action, action_type)
                    )
    ...
    def generate_ros_system_model_list(
        self,
        components,
        ros_system_model_file,
        ros_model_file="",
        print_param_value=True,
    ):
        sucess, ros_system_model_str = self.create_ros_system_model_list(components)
        with open(ros_system_model_file, "w") as outfile:
            outfile.write(ros_system_model_str)
            ...
            if ros_model_file:
                ...
                rosmodel_generator.generate_ros_model_from_system(
                    self.system, self.system.package, ros_model_file, print_param_value
                )
