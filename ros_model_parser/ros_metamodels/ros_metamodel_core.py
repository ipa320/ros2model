...
class Parameter(object):
    def __init__(
        self, name, value=None, type=None, default=None, namespace="", print_value=True
    ):
        self.fullname = name
        self.namespace = namespace
        self.name = name[len(self.namespace) - 1 :]
    ...
    def set_value(self, value, indent):
        ...
        if self.type == "String":
            ...
            str_param_value += str(value).replace("[", "{").replace("]", "}")
    ...
    def types_struct(self, struct_dict, indent):
        ...
        for struct_element in struct_dict:
            ...
            if isinstance(sub_value, dict):
                str_param += self.types_struct(struct_dict[struct_element], indent_new)
            str_param += ",\n"
...
class InterfaceSet(set):
    ...
    def dump_xtext_model(
        self, indent="", name_type="", interface_type="", name_block=""
    ):
        if len(self) == 0:
            return ""
        str_ = ("\n%s%s {\n") % (indent, name_block)
        for elem in self:
            str_ += (
                elem.dump_xtext_model(indent + "  ", name_type, interface_type) + ",\n"
            )
        str_ = str_[:-2]
        str_ += "}"
        return str_
